...
const Booking = ({ searchParams }) => {
    ...
    return (
        <div className='pt-20'>
                    ...
                    {steps?.map((item, index) => (
                        <div key={index}>
                            <div className='flex items-center gap-5'>
                                ...
                                {index !== 3 && (
                                    <div className={`w-full h-0.5 ${current > index + 1 ? "bg-third-color" : "bg-black"} rounded`}>
                                 </div>
                                )}
                            </div>
                            <p className='w-full font-roboto font-semibold text-sm uppercase my-2 text-slate-400'>Step {index + 1}</p>
                            <p className='w-full font-roboto font-semibold text-sm uppercase text-slate-400'>Step {item.name}</p>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className={`${current == 4 ? "hidden":"flex"} justify-between items-center px[10%] py-3`}>
                <button className='bg-slate-100 px-10 py-3 rounded-full'>Back</button>
                <Link href={`/booking?index=${current + 1}`} className={`bg-third-color text-white px-10 py-3 rounded-full`}>Next</Link>
            </div>
        </div>
    )
}
