...
const Header = () => {
  ...
  return (
    <div className='bg-white shadow-lg h-16 flex justify-between items-center md:px-[10%]'>
      ...
      <div className={`${isMenu ?"flex flex-col absolute top-16 w-full":"hidden md:flex"} gap-5 md:flex-row md:static bg-white md:w-auto text-center`}> 
        {links?.map((link)=>(
            <div key={link.link} >
                <Link
                className={`${pathName == link.link? "text-third-color":"text-gray-400"}`} href={link.link} exact={link.exact} > 
                <span>{link.text}</span>
                </Link>
            </div>
        ))}
